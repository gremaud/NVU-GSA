...
def set_algebraic_variables(p,u, t, idx, input_data, total_waveform, location):
...
############################################################################            
    ...
    ####
    a.J_VOCC_i = p.G_Ca_i * (v.v_i - p.v_Ca1_i) / (1 + np.exp(-(v.v_i - p.v_Ca2_i) / p.R_Ca_i)) 
    ####
...
#    #%% 
    ...
    a.J_CICR_k = p.C_k * v.s_k**2 / (p.s_c_k**2 + v.s_k**2) * v.Ca_k**4 / (p.c_c_k**4 + v.Ca_k**4)
